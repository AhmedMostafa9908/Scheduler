...
"border-radius:12;")
        ...
        self.label_4.setGeometry(QtCore.QRect(710, 90, 291, 61))
...
"border-radius:12;")
        ...
        self.radioButton.setGeometry(QtCore.QRect(940, 160, 211, 61))
...
"""self.draw(Processes_Names[0], last_x, Arrival_Time_List[1])
last_x = last_x + Arrival_Time_List[1]
Burst_Time_List[0]=Burst_Time_List[0]-(Arrival_Time_List[1]-Arrival_Time_List[0])
for z in range(len(Processes_Names)):
if(flag==1) :
                    z=z-1
                    flag=0
                if(last_x >= Arrival_Time_List[z]):
                  self.draw(Processes_Names[z], last_x, Burst_Time_List[z] + last_x)
                  last_x = last_x + Burst_Time_List[z]
                else :
                  if(z < len(Processes_Names)-1) :
                    self.swapPositions(Arrival_Time_List, z, z+1)
                    self.swapPositions(Burst_Time_List, z, z+1)
                    self.swapPositions(Processes_Names, z, z+1)
                    flag=1
...
New_list_burst=[]
New_list_Process=[]
total_time = sum(Burst_Time_List)
k =0
while ( k < total_time):
                if k in Arrival_Time_List:
                    index_Of_Arrived = Arrival_Time_List.index(k)
                    Arrived_List.append(Burst_Time_List[index_Of_Arrived])
                min_value = self.get_min_pos(Arrived_List)
                index_Of_min = Arrived_List.index(min_value)
                New_list_burst.append(Arrived_List[index_Of_min])
                New_list_Process.append(Processes_Names[index_Of_min])
                Arrived_List[index_Of_min]=Arrived_List[index_Of_min]-0.5
                k=k+0.5
                for z in range(len(New_list_Process)):
                    self.draw(New_list_Process[z], last_x, New_list_burst[z] + last_x)
                    last_x = last_x + New_list_burst[z]"""